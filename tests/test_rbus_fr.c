    ...
    rbusEventSubscription_t* subscription)
{
	...
	WebcfgInfo("rbusEvent_OnPublish success callback received successfully with message %s\n", inVal);
	CU_ASSERT(1);
    }
    (void)handle;
}
...
void test_webcfgSubdocForceResetSet_GetHandler(void)
{
    ...
    int ret = rbusEvent_Subscribe(rbus_handle, WEBCFG_SUBDOC_FORCERESET_PARAM, publishEventSuccessCallbackHandler, NULL, 0);
...
void test_publishSubdocResetEvent()
{
	...
	regWebConfigDataModel();
	int rc = rbusEvent_Subscribe(rbus_handle, WEBCFG_SUBDOC_FORCERESET_PARAM, publishEventSuccessCallbackHandler, NULL, 0);
...
void test_webcfg_util_method()
{
	...
	while(checkParams)
	{
		...
		{
			char const* propGetValue = (char const*)rbusValue_GetBytes(propValue, NULL);
			CU_ASSERT_STRING_EQUAL(propName, "data");
			CU_ASSERT_STRING_EQUAL(propGetValue, "mocaBLOB");
		}*/
		checkParams = rbusProperty_GetNext(checkParams);	
	}
    	rbusObject_Release(inParams);
